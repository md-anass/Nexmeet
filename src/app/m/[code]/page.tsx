...
type PublicMeeting = { title: string; public_code: string; status: "active" | "ended"; host_display_name: string | null };
...
export default async function MeetingLobbyPage({ params }: PageProps) {
  ...
  const meeting = meetingData as PublicMeeting | null;
  if (error || !meeting) notFound();

  const user = await getAuthenticatedUser();
  const { data: ownedMeeting } = user
    ? await supabase.from("meetings").select("host_user_id").eq("public_code", code).maybeSingle()
    : { data: null };
  const isHost = Boolean(user && ownedMeeting?.host_user_id === user.id);
  const hostName = meeting.host_display_name?.trim() || "Host";
  ...
  const shareLink = host ? `${protocol}://${host}/m/${meeting.public_code}` : `/m/${meeting.public_code}`;

  if (meeting.status === "active" && participantSession) {
    return <ParticipantMeeting meetingCode={meeting.public_code} meetingTitle={meeting.title} displayName={participantSession.display_name} autoReconnect={participantSession.status === "joined"} isHost={isHost} shareLink={shareLink} />;
  }
            ...
            <>
              ...
              {participantSession ? (
                <>
                  ...
                  <ParticipantMeeting meetingCode={meeting.public_code} meetingTitle={meeting.title} displayName={participantSession.display_name} autoReconnect={participantSession.status === "joined"} />
                </>
              ) : (
                <GuestJoinForm meetingCode={meeting.public_code} />
              )}
              <p className="mt-3 text-center text-xs text-slate-500">Meeting code: <span className="font-semibold tracking-wide text-slate-700">{meeting.public_code}</span></p>
