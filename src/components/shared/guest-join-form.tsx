...
export function GuestJoinForm({ meetingCode, approvalRequired = false }: { meetingCode: string; approvalRequired?: boolean }) {
  ...
  return (
    <form action={formAction} className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-5">
      ...
      <button type="submit" disabled={pending} className="mt-4 w-full rounded-xl bg-slate-950 px-5 py-3 text-sm font-semibold text-white hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-60">{pending ? "Joining..." : approvalRequired ? "Request to join" : "Join meeting"}</button>
    </form>
  );
}
