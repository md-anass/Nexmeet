...
export function NexMeetMeetingLoader({ label = "Connecting to your meeting..." }: { label?: string }) {
  ...
  return <div className="flex min-h-[100dvh] items-center justify-center bg-[#020817] px-6 pt-[env(safe-area-inset-top)] text-center text-white">
    {reducedMotion || showStatic ? <NexMeetBrand /> : <video src="/brand/animation (2).mp4" autoPlay muted playsInline aria-label="NexMeet" className="h-36 w-36 rounded-3xl object-cover" onError={() => setShowStatic(true)} onEnded={() => setShowStatic(true)} />}
