...
export function ChatPanel({ open, meetingCode, participantSelector, participants, localParticipant, onClose, onUnreadChange }: { open: boolean; meetingCode: string; participantSelector: string; participants: Participant[]; localParticipant: Participant; onClose: () => void; onUnreadChange: (count: number) => void }) {
  ...
  return <aside id="nexmeet-chat-panel" aria-label="Chat" className={`absolute inset-x-0 bottom-[calc(4.5rem+env(safe-area-inset-bottom))] z-40 flex h-[calc(100%_-_4.5rem_-_env(safe-area-inset-bottom))] max-h-[calc(100%_-_4.5rem_-_env(safe-area-inset-bottom))] min-h-0 flex-col overscroll-contain rounded-t-[28px] border border-white/[0.12] bg-[#09142a]/[0.97] pb-0 text-white shadow-[0_-20px_65px_rgba(0,0,0,.42)] backdrop-blur-xl sm:inset-y-0 sm:left-auto sm:right-0 sm:h-auto sm:max-h-none sm:w-[23rem] sm:rounded-none sm:border-b-0 sm:border-r-0 sm:border-t-0 sm:pb-0 sm:shadow-[-22px_0_65px_rgba(0,0,0,.25)] ${open ? "translate-y-0 opacity-100" : "pointer-events-none translate-y-full opacity-0 sm:translate-x-full sm:translate-y-0"} transition duration-200`}>
    <header className="flex items-center justify-between border-b border-white/[0.08] px-5 py-5"><div><h2 className="text-lg font-semibold tracking-[-0.025em]">Chat</h2><p className="mt-1 text-xs font-medium text-slate-400">{messages.length} message{messages.length === 1 ? "" : "s"}</p></div><button type="button" onClick={onClose} aria-label="Close chat" title="Close chat" className="rounded-xl p-2 text-slate-300 hover:bg-white/10 focus:outline-none focus:ring-2 focus:ring-cyan-300/60"><X className="size-5" /></button></header>
    <div className="border-b border-white/[0.08] px-5 py-4"><label className="flex items-center gap-2 text-sm text-slate-300">To:<select value={effectiveRecipient ?? ""} onChange={(event) => setRecipient(event.target.value || null)} className="min-w-0 flex-1 rounded-xl border border-white/10 bg-white/[0.055] px-3 py-2.5 text-sm text-white outline-none transition focus:border-cyan-300/45 focus:ring-2 focus:ring-cyan-300/20"><option value="" className="bg-[#09142a]">Everyone</option>{remoteParticipants.map((participant) => <option key={participant.identity} value={participant.identity} className="bg-[#09142a]">{participant.name || "Participant"}</option>)}</select></label></div>
