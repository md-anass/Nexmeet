...
export function ParticipantTile({ participant, track, displayName, isLocal = false, cameraEnabled, microphoneEnabled = true, isHost = false, handRaised = false, reaction, className = "" }: ParticipantTileProps) {
  ...
  return <div className={`group relative aspect-video min-h-0 w-full overflow-hidden rounded-[22px] border bg-[#0a1327] shadow-[0_24px_70px_rgba(0,0,0,.28)] ${isLocal ? "border-cyan-300/65 shadow-[0_24px_70px_rgba(0,0,0,.28),0_0_0_1px_rgba(34,211,238,.12),0_0_45px_rgba(6,182,212,.13)]" : "border-violet-300/20"} ${className}`}>
    ...
    <div className="absolute inset-x-4 bottom-4 flex items-center justify-between gap-2"><span className="rounded-xl border border-white/10 bg-[#020817]/70 px-3 py-1.5 text-xs font-semibold text-white shadow-lg backdrop-blur-md">{name}{isLocal && <span className="ml-1.5 text-cyan-300">You</span>}</span><span className="flex items-center gap-1.5">{isHost && <span className="rounded-lg border border-cyan-300/15 bg-cyan-300/15 px-2 py-1 text-[10px] font-semibold text-cyan-100">Host</span>}{!microphoneEnabled && <span className="rounded-lg bg-red-500/85 p-1.5 text-white shadow-lg" title="Microphone off"><MicOff className="size-3" /></span>}</span></div>
  </div>;
}
