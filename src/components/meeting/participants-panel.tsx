...
export function ParticipantsPanel({ participants, localParticipant, displayName, isHost, onClose }: ParticipantsPanelProps) {
  return <>
    ...
    <aside aria-label="Participants" className="fixed inset-x-0 bottom-0 z-40 max-h-[min(78dvh,42rem)] rounded-t-3xl border border-white/10 bg-[#081126]/95 p-4 pb-[max(1rem,env(safe-area-inset-bottom))] text-white shadow-2xl shadow-black/50 backdrop-blur-xl sm:inset-y-0 sm:left-auto sm:right-0 sm:max-h-none sm:w-80 sm:rounded-none sm:border-b-0 sm:border-r-0 sm:border-t-0 sm:p-5 sm:pb-5">
      ...
      <ul className="mt-5 max-h-[calc(min(78dvh,42rem)-6rem)] space-y-2 overflow-y-auto pr-1 sm:max-h-[calc(100dvh-8rem)]">
